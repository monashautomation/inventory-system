...
export function FilamentByProject() {
  const [days, setDays] = useState<DaysOption>(30);

  const { data, isLoading } = trpc.printStats.filamentByProject.useQuery({
    days,
  });
  ...
  return (
    <Card className="flex flex-col">
      <CardHeader className="flex flex-row items-start justify-between space-y-0 pb-2">
        <div>
          <CardTitle>Filament by Project</CardTitle>
          <CardDescription>Filament used per project by weight</CardDescription>
        </div>
        ...
        {isLoading ? (
          ...
          <ol className="space-y-3">
            {data.slice(0, 8).map((entry, i) => (
              <li key={entry.projectName} className="space-y-1">
