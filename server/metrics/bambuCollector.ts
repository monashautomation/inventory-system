...
function processAmsUnit(
  ...
  for (const tray of unit.tray) {
    ...
    if (tray.cali_idx != null) trayMap.set("ams_tray_cali_idx", tray.cali_idx);
    if (tray.nozzle_temp_min != null)
      trayMap.set("ams_tray_nozzle_temp_min_celsius", tray.nozzle_temp_min);
    if (tray.nozzle_temp_max != null)
      trayMap.set("ams_tray_nozzle_temp_max_celsius", tray.nozzle_temp_max);
...
function processVtTray(store: BambuPrinterStore, tray: AMSTray): void {
  ...
  if (tray.cali_idx != null) store.gauges.set("vt_tray_cali_idx", tray.cali_idx);
  if (tray.nozzle_temp_min != null)
    store.gauges.set("vt_tray_nozzle_temp_min_celsius", tray.nozzle_temp_min);
  if (tray.nozzle_temp_max != null)
    store.gauges.set("vt_tray_nozzle_temp_max_celsius", tray.nozzle_temp_max);
}
...
function processApiStatus(
  ...
  if (temps.nozzle != null) store.gauges.set("nozzle_temperature", temps.nozzle);
  ...
  if (temps.chamber != null) store.gauges.set("chamber_temperature", temps.chamber);
  ...
  if (status.layer_num != null) store.gauges.set("current_layer", status.layer_num);
  if (status.total_layers != null) store.gauges.set("total_layers", status.total_layers);
...
const PRINTER_GAUGES: [string, string][] = [
  ...
  ["gcode_state", "G-code state (0=idle, 1=running, 2=paused, 3=finished, 4=failed/error)"],
  ...
  ["upgrade_state_status", "Upgrade status (0=idle, 1=upgrading, 2=completed, 3=error)"],
  ...
  ["upload_status", "Upload status (0=idle, 1=uploading, 2=completed, 3=error)"],
...
async function pollMetrics(): Promise<void> {
  try {
    ...
    console.error("[bambu-metrics] Poll failed:", err instanceof Error ? err.message : err);
  }
}
...
export function initBambuMetricsListener(): void {
  ...
  void pollMetrics();
  pollHandle = setInterval(() => void pollMetrics(), METRICS_POLL_INTERVAL_MS);
  ...
  for (const [suffix, help] of AMS_UNIT_GAUGES) {
    ...
    for (const entry of metricsCache.values()) {
      for (const [amsId, amsMetrics] of entry.store.amsUnit) {
        ...
        if (value !== undefined) {
          samples.push({ value, labels: { printer: entry.printerName, ams: amsId } });
        }
      }
    }
    if (samples.length > 0) {
      ...
      for (const [amsId, trayMap] of entry.store.amsTray) {
        for (const [trayId, trayMetrics] of trayMap) {
          ...
          if (value !== undefined) {
            samples.push({
              value,
              labels: { printer: entry.printerName, ams: amsId, tray: trayId },
            });
          }
        }
      }
